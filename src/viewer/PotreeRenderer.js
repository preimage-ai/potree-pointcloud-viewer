...
export class PotreeRenderer {
	...
	splitRender(params){
		...
		const camera = params.camera ? params.camera : viewer.scene2.getActiveCamera();
		...
		if(viewer.background === "skybox"){
			...
			viewer.skybox.camera.updateProjectionMatrix();
			renderer.render(viewer.scene2.scene, viewer.scene2.cameraP);
		}else if(viewer.background === "gradient"){
			renderer.render(viewer.scene2.sceneBG, viewer.scene2.cameraBG);
		}

		for(let pointcloud of this.viewer.scene2.pointclouds){
			const {material} = pointcloud;
			material.useEDL = false;
		}

		viewer.pRenderer.render(viewer.scene2.scenePointCloud, camera, null, {
			clipSpheres: viewer.scene2.volumes.filter(v => (v instanceof Potree.SphereVolume)),
		});
		
		// render scene
		renderer.render(viewer.scene2.scene, camera);

		viewer.dispatchEvent({type: "render.pass.scene",viewer: viewer});
		
		viewer.clippingTool.update();
		renderer.render(viewer.clippingTool.sceneMarker, viewer.scene2.cameraScreenSpace); //viewer.scene.cameraScreenSpace);
		renderer.render(viewer.clippingTool.sceneVolume, camera);
